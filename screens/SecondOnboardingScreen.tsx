...
const SecondOnboardingScreen = ({ navigation }) => {
  ...
  const handleShirtsPressed = () => setShirtsPressed(current => !current);
  const handlePantsPressed = () => setPantsPressed(current => !current);
  const handleShoesPressed = () => setShoesPressed(current => !current);
  const handleHoodiesPressed = () => setHoodiesPressed(current => !current);
  const handleHatsPressed = () => setHatsPressed(current => !current);
  const handleSweatersPressed = () => setSweatersPressed(current => !current);
  const handleJewelleryPressed = () => setJewelleryPressed(current => !current);
  const handleJacketsPressed = () => setJacketsPressed(current => !current);
  const handleShortsPressed = () => setShortsPressed(current => !current);
  const handleGlovesPressed = () => setGlovesPressed(current => !current);
